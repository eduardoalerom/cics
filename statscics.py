...
for entorno in selected_envs:
    ...
    # Detalle Diario
    st.markdown("---")
    st.subheader(f"📊 Detalle por día - {dia_seleccionado}")
    ...
    for cat in categorias:
        ...
        # Gráfica multilínea para TRANSACCION y Web Services
        multi = ['TRANSACCION', 'WEB SERVICES', 'WEBSERVICE', 'WEBSERVICES']
        if cat.upper() in multi:
            ...
            # Línea de tendencia para CPU (antes MIPS)
    ...
    for cat in categorias:
        ...
        # Identificar día pico y anotarlo en la gráfica
        ...
        fig2.add_trace(
            go.Scatter(
                ...
                text=[f"Pico: {pico['DIA_SEM']} ({pico['USO']:,.0f})"],
